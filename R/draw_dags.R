...
p_dag1 <- ggplot() + 
  ...
  geom_dag_text(
    data = df_dag1 %>% filter(type != "target"),
    aes(x = x,
        y = y,
        color = interest_node), 
...
p_dag2 <- ggplot() + 
  ...
  geom_dag_text(
    data = df_dag2 %>% filter(type != "target"),
    aes(x = x,
        y = y,
        color = interest_node),
    # sort them alphabetically
  ...
  geom_dag_edges_link(
    data = df_dag2 %>% 
      mutate(
        across(.cols = c(xend, yend), ~ifelse(to == "barepost", .x-0.05, .x))
      ),
    mapping = aes(
      edge_color = interest_edge,
      xend = xend+0.03,
...
p_dag3 <- ggplot() + 
  ...
  geom_dag_edges_link(
    data = df_dag3 %>% 
      mutate(xend = ifelse(to == "out", xend-0.02, xend)),
    mapping = aes(
      edge_color = interest_edge,
      xend = xend,
      yend = yend,
...
# export
png("results/plot_dag_collider.png", width = 1600, height = 900, res = 280)
p_dag3
dev.off()
